...
void
MouseObserver(uint8_t buttons, int8_t displacement_x, int8_t displacement_y) {
...
void
SwitchEhci2Xhci(const pci::Device& xhc_dev) {
...
KernelMainNewStack(const FrameBufferConfig& frame_buffer_config_ref,
    ...
    if (auto err = screen.Initialize(frame_buffer_config)) {
        Log(kError,
            "failed to initialize frame buffer: %s at %s:%d\n",
            err.Name(),
            err.File(),
            err.Line());
    }
