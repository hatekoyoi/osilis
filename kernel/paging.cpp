...
namespace {
    ...
    alignas(kPageSize4K) std::array<uint64_t, 512> pm14_table;
...
SetupIdentityPageTable() {
    pm14_table[0] = reinterpret_cast<uint64_t>(&pdp_table[0]) | 0x003;
    for (int i_pdpt = 0; i_pdpt < page_directory.size(); ++i_pdpt) {
        ...
        for (int i_pd = 0; i_pd < 512; ++i_pd) {
            page_directory[i_pdpt][i_pd] = i_pdpt * kPageSize1G + i_pd * kPageSize2M | 0x003;
        }
    }

    SetCR3(reinterpret_cast<uint64_t>(&pm14_table[0]));
}
