...
Console::PutString(const char* s) {
    while (*s) {
        if (*s == '\n') {
            ...
            WriteAscii(
                *writer_, Vector2D<int>{ 8 * cursor_column_, 16 * cursor_row_ }, *s, fg_color_);
...
void
Console::SetWriter(PixelWriter* writer) {
    if (writer == writer_) {
        return;
    }
    writer_ = writer;
...
void
Console::SetWindow(const std::shared_ptr<Window>& window) {
    if (window == window_) {
        return;
    }
    window_ = window;
...
void
Console::SetLayerID(unsigned int layer_id) {
    layer_id_ = layer_id;
}

unsigned int
Console::LayerID() const {
    return layer_id_;
}
